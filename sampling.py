...
N_PROCESSES = 1
SAMPLE_STEPS = 1
EXCHANGE_STEPS = SAMPLE_STEPS * 50
N_STEPS = 10000
TEMPERATURE = 500   # K
...
for o in range(STATE_NUM):
    ...
    plt.hist(x_data, bins=100, color=color_list[o % len(color_list)], alpha=0.5)
...
for o in range(STATE_NUM):
    u_data = sample_objs[o].potentials
    plt.hist(u_data, bins=100, color=color_list[o % len(color_list)], alpha=0.5)
plt.gca().set(title='Frequency Histogram', ylabel='Frequency')
plt.savefig(Path(f"./figures/potential_hist_ex_{EXCHANGE_STEPS}_samples_{N_STEPS}_states_{STATE_NUM}.png"))
...
mbar_estimator = _MBAR(method="L-BFGS-B").fit(pd.concat([u_nk for u_nk in org_u_nks]))
f_k = [0.0]
